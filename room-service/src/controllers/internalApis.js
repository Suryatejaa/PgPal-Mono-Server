...
// Internal API Error Tracker for Room Service
const internalErrorTracker = {
    ...
    addError: (error) => {
        const errorEntry = {
            id: `room_internal_err_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        if (error.status >= 500 || error.timeout) {
            console.error(`🚨 [ROOM INTERNAL API] CRITICAL ERROR in ${error.sourceService} -> ${error.targetService}`);
...
const makeInternalApiCall = async (
    ...
    try {
        ...
        const config = {
            ...
            url,
            timeout: 10000, // 10 second timeout for room service
            headers: {
        ...
        // Log successful call
        console.log(`✅ [ROOM INTERNAL API] ${sourceService} -> ${targetService}: ${method} ${url} (${duration}ms)`);
