...
// Internal API Error Tracker for Notification Service
const internalErrorTracker = {
    ...
    addError: (error) => {
        const errorEntry = {
            id: `notification_internal_err_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        if (error.status >= 500 || error.timeout) {
            console.error(`🚨 [NOTIFICATION INTERNAL API] CRITICAL ERROR in ${error.sourceService} -> ${error.targetService}`);
...
const makeInternalApiCall = async (
    ...
    try {
        ...
        const config = {
            ...
            url,
            timeout: 8000, // 8 second timeout for notification service
            headers: {
        ...
        // Log successful call
        console.log(`✅ [NOTIFICATION INTERNAL API] ${sourceService} -> ${targetService}: ${method} ${url} (${duration}ms)`);
...
const sendNotification = async (currentUser, tenantId, title, message, type, method) => {
    // Note: This is a self-referencing function for the notification service
    // Usually this would be an external call, but keeping for API consistency
    console.log(`📧 [NOTIFICATION SERVICE] Sending notification to ${tenantId}: ${title}`);
    return true; // Always return success for self-notifications
};
